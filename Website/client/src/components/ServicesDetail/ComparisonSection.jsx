              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200 bg-white dark:bg-black">
                  {data.rows.map((row, index) => (
                    ...
                    >
                      {/* Feature Column */}
                      <td className="whitespace-nowrap px-4 py-4 text-sm sticky left-0 z-10 bg-white dark:bg-black">
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-200">
            <div className="inline-flex min-w-full">
              <div className="flex flex-col">
                ...
                {data.rows.map((row, index) => (
                  <div key={index} className="sticky left-0 z-10 bg-white dark:bg-black p-3 min-w-[150px] border-b border-gray-200">
              ...
              {data.columns.slice(1).map((col, colIndex) => (
                <div key={colIndex} className="flex flex-col">
                  ...
                  {data.rows.map((row, rowIndex) => (
                    <div key={rowIndex} className="bg-white dark:bg-black p-3 min-w-[120px] border-l border-b border-gray-200">
                      <div className="text-sm text-gray-800 dark:text-gray-200 text-center">{row.values[colIndex]}</div>
                    </div>
                  ))}
                </div>
              ))}
