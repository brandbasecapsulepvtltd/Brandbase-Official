...
const ArticleCard = ({ blog }) => (
  <a href={`/blogs/${blog.metadata.category}/${blog.metadata.slug}`} className="group block h-full">
    ...
    <div className="pt-2">
      ...
      <h3 className="text-base font-bold mb-1 leading-snug text-gray-900 dark:text-white group-hover:text-orange-600 transition">
        {blog.metadata.title}
      </h3>
...
const ReusableContentSection = ({ allBlogs = [] }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 dark:bg-black py-12 md:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          return (
            <div key={category} className={`${index > 0 ? 'mt-12 md:mt-20' : ''}`}>
              ...
              <div className="flex justify-between items-center mb-6 md:mb-8">
                ...
                <a href={`/blog/${category}`} className="text-sm font-semibold text-orange-600 hover:text-orange-800 transition">
                  View all
                </a>
              </div>
              ...
              {index < categories.length - 1 && (
                <hr className="mt-12 md:mt-20 border-gray-200 dark:border-zinc-800" />
              )}
            </div>
          );
        })}
