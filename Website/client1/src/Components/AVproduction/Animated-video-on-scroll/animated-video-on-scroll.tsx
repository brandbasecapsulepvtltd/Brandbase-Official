...
> = ({ children, className, ...props }) => {
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div
        ref={scrollRef}
        className={cn("relative min-h-svh w-full bg-white", className)}
        {...props}
      >
        {children}
      </div>
...
>(({ className, transition, ...props }, ref) => {
  return (
    <motion.button
      ...
      ref={ref}
      className={cn(
        "group relative flex w-fit items-center rounded-full border border-blue-600 bg-white px-6 py-3 text-blue-600 shadow-lg transition-all duration-300 hover:bg-blue-600 hover:text-white hover:shadow-xl",
        className
      )}
      {...props}
    />
